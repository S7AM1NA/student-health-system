...
6. **UserHealthGoal (用户健康目标)**

| 字段名 (Field)                  | 数据类型 (Type)        | 说明                   | 备注                             |
| ------------------------------- | ---------------------- | ---------------------- | -------------------------------- |
| `user`                          | `OneToOneField`        | 所属用户               | 关联到`CustomUser`，每个用户唯一 |
| `target_sleep_duration`         | `FloatField`           | 目标睡眠时长(小时)     |                                  |
| `target_sport_duration_minutes` | `PositiveIntegerField` | 目标运动时长(分钟)     |                                  |
...
### **5. 用户健康目标 (Health Goals)**

- **Endpoint**: /api/goals/
- **核心**: 管理当前登录用户的个人每日健康目标。如果用户没有目标，系统会在首次访问时自动创建。

| 操作              | Method | URL         | 说明                                   |
| ----------------- | ------ | ----------- | -------------------------------------- |
| **获取健康目标**  | GET    | /api/goals/ | 获取当前用户的健康目标。               |
| **创建/更新目标** | PUT    | /api/goals/ | 创建或更新用户的目标（支持部分更新）。 |
...
### **11. 周度睡眠数据报告 (Weekly Sleep Report)**
...
### **12. 周期性综合健康报告 (Periodic Health Report)**
...
### **13. 健康异常预警 (Health Alerts)**

- **Endpoint**: /api/alerts/check/

- **Method**: GET

- **核心功能**: 检查用户近期的健康数据，识别是否存在连续多天的负面趋势（如连续睡眠不足、连续运动量过低）。此接口设计为轻量级，适合在应用启动或进入主页时调用，以决定是否需要向用户弹出提醒。

- **Success Response (200 OK)**:

  - 响应体是一个**数组**，包含所有当前触发的预警对象。
  - **如果没有任何预警，将返回一个空数组 []。**

- **响应 Body (示例：当所有预警都触发时)**:

  ```json
  [
      {
          "alert_code": "POOR_SLEEP_STREAK",
          "message": "您已连续3天睡眠不足7.0小时，请注意保证充足的休息。"
      },
      {
          "alert_code": "LOW_ACTIVITY_STREAK",
          "message": "您已连续3天运动量过低，建议增加适度锻炼来保持活力。"
      }
  ]
  ```

  - **字段说明**:
    - alert_code: 预警的唯一标识符。前端可以根据此代码执行特定逻辑，例如显示不同的图标或样式。
    - message: 准备好的、可以直接显示给用户的预警文本。
